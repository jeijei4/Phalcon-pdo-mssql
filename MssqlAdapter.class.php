...
/**
 * Class Mssql, MssqlDialect
 * Connecting to Microsoft SQL Server PDO driver With Phalcon Framework
 * Original work by Davide Airaghi (www.airaghi.net): https://github.com/davideairaghi/php/tree/master/library/Airaghi/PhalconPHP/MSSQL
 * Modified by JeiHO
 * Version: 0.0.1
 * Tested with: Phalcon V3.4.1, PHP V5.6.38, Sql server 2005
 */
class MssqlDialect extends \Phalcon\Db\Dialect //implements \Phalcon\Db\DialectInterface
{
    /*
...
class Mssql extends AdapterPdo implements EventsAwareInterface, AdapterInterface
{
    ...
    public function connect(array $descriptor = null)
    {
        ...
        if (stripos($descriptor['pdoType'], 'dblib') !== false) {
            $serverDB = 'host';
            $nameDB = 'dbname';
            $options = ";Encrypt=true;TrustServerCertificate=true;charset=utf8";
        } else {
            $serverDB = 'server';
            $nameDB = 'database';
            $options = '';
        ...
        $this->_pdo = new \PDO(
            $descriptor['pdoType'] . ':' .
            $serverDB . '=' . $descriptor['host'] . $portDB . ';' .
            $nameDB . '=' . $descriptor['dbname'] . ';' .
